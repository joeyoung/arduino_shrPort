...
|| #
     ...
     For Keypad_exp and shrPrta, GDY - May 26/20
     Verify with TCA6408, alternate I2C port - DynamicKeypad_expTCalt - July 9/20
*/
#include <shrPrta.h>
...
shrPrta prt( I2CADDR, TCA6408, &jwire);     // specify alternate I2C bus
//create a new Keypad
Keypad_exp keypad(&prt, keypadMap,rowPins,colPins,sizeof(rowPins),sizeof(colPins) );

unsigned long startTime;
const byte ledPin = 13;	                     // Use the LED on pin 13.
const byte i2cledPin = 7;                   // for LED on unused i2c port pin
...
void loop() {
    ...
    if (alpha && millis()-startTime>100) {           // Flash the LED if we are using the letter keymap.
      ...
      prt.pin_write( i2cledPin, !keypad.pin_read( i2cledPin ) );  // also on i2c port pin
		  startTime = millis();
    }
}
    ...
    case HOLD:
        if (key == '#')  {                   // Toggle between keymaps.
            if (alpha == true)  {            // We are currently using a keymap with letters
                word saveDDR = prt.iodir_read( ); // begin loses current ddr state
                keypad.begin(*numberKeys);   // and want to change to numbers.
                prt.iodir_write( saveDDR );
                alpha = false;
