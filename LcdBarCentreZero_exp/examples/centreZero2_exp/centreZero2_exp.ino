...
LiquidCrystal_exp lcd( &prt, 16, 2 ); //instance at I2C port 38, 16 char, 2 rows
...
//  lcd.begin( 2, lcdNumCols );
  Wire.begin( );
  prt.begin( );
  for( byte ix=0; ix<8; ix++ ) prt.pin_mode( ix, OUTPUT );  //lcd expects port setup 
  lcd.init();
  zcb.loadCG( );
